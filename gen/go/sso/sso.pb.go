...
type RegisterRequest struct {
	...
	Email         string                 `protobuf:"bytes,1,opt,name=email,proto3" json:"email,omitempty"`       // Email пользователя, на который будет создан аккаунт (логин).
	Password      string                 `protobuf:"bytes,2,opt,name=password,proto3" json:"password,omitempty"` // Пароль для нового аккаунта. Обычно передаётся по защищённому каналу (TLS) и хранится в виде хэша.
...
type RegisterResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        int64                  `protobuf:"varint,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"` // Идентификатор созданного пользователя (ID), который можно хранить у себя и использовать в дальнейших запросах.
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type LoginRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Email         string                 `protobuf:"bytes,1,opt,name=email,proto3" json:"email,omitempty"`               // Email пользователя, который используется как логин при входе.
	Password      string                 `protobuf:"bytes,2,opt,name=password,proto3" json:"password,omitempty"`         // Пароль пользователя для проверки при входе (валидируется на сервере).
	AppId         int32                  `protobuf:"varint,3,opt,name=app_id,json=appId,proto3" json:"app_id,omitempty"` // ID приложения/клиента, для которого выполняется вход (например, web, mobile, admin).
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type LoginResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Token         string                 `protobuf:"bytes,1,opt,name=token,proto3" json:"token,omitempty"` // Токен доступа (например JWT). Клиент обычно отправляет его в последующих запросах для авторизации.
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
